# let's cheat and save these screens elsewhere

# NEW FUNCTIONS -----------------------------------------------------------

# add new obligatory arguments: MGP_quantities and MGP_quantities_accepted
carteltraintest_new <- function (collusion, calculated_screens, trainshare = 0.75, 
                                                                   ...
                                                                   "SL.glmnet", "SL.ksvm", "SL.glm")) 
{
  ...
  pb = progress::progress_bar$new(total=nsim, format=":elapsedfull Simulation :current of :total [:bar] :percent :eta", clear=FALSE)
  for (j in 1:nsim) {
...
screens_new <- function (prices, MGP_quantities = NULL, MGP_quantities_accepted = NULL, partial = FALSE, numberbids = FALSE) 
{
  x = c()
  
  # add a progress bar
  pb = progress::progress_bar$new(total=nrow(prices), format = ":elapsedfull Tender :current of :total [:bar] :percent :eta", clear=FALSE)
