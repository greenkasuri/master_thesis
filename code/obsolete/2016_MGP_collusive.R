...
# remove rows with fewer than 2 offers (not including 0s)
equal_size_sample <- equal_size_sample[apply(equal_size_sample[,-1], 1, function(x) sum(x > 0, na.rm=TRUE) >= 2), ]

setwd("C:\\Users\\jerep\\OneDrive\\Documents\\thesis\\master_thesis\\GME\\intermediate_results\\data")
save(equal_size_sample, file="2016_MGP_collusive_data.RData")
