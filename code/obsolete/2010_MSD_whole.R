...
df_relevant <- function(){df[PURPOSE_CD=="OFF" & SCOPE=="AS" & (STATUS_CD=="ACC" | STATUS_CD=="REJ")]}
...
sampled_units <- df_relevant() %>%
  ...
  .[ENERGY_PRICE_NO <= 1000 & !is.na(ENERGY_PRICE_NO)] %>%
...
preprocessed <- df_relevant() %>%
  ...
  # arrange in descending order to sort NA's to the right
...
save(equal_size_sample, file="2010_MSD_whole_data.RData")
...
save(model, file="2010_MSD_whole_model.RData")
save(model_demeaned, file="2010_MSD_whole_model_demeaned.RData")
save(model_standardized, file="2010_MSD_whole_model_standardized.RData")
