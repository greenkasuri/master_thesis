...
# function to randomly remove if the unit is not collusive
remove_random_non_collusive <- function(dt){
  # split into collusive and non-collusive tables
  collusive <- dt["UNIT_REFERENCE_NO" %in% collusive_units]
  non_collusive <- dt[!("UNIT_REFERENCE_NO" %in% collusive_units)]
  
  no_collusive <- nrow(collusive)
  
  # randomly remove to the size of 25 minus collusive size (total size = 25)
  sample <- slice_sample(non_collusive, n = 25-no_collusive)
  
  return(rbind(collusive, sample))
}
...
preprocessed <- df_relevant() %>%
  ...
  # randomly sample 25 rows, including the collusive units
  group_modify(~remove_random_non_collusive(.x)) %>%
...
save(equal_size_sample, file="2016_MGP_whole_data.RData")
