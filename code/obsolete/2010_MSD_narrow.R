...
holidays <- c(20100101, 20100106, 20100404, 20100405, 20100501, 20100602, 20101101, 20101208, 20101225)

collusive_units <- c("UP_CNTRLDTVRL_1", "UP_SPARANISE_1", "UP_SPARANISE_2", "UP_NAPOLIL_4")

# relevant market
df_relevant <- function(){df[
  # collusive units only
  UNIT_REFERENCE_NO %in% collusive_units &
    ...
    # holidays or Sundays
    (BID_OFFER_DATE_DT %in% holidays | wday(ymd(BID_OFFER_DATE_DT))==1) &
...
train_data <- preprocessed[, .SD[sample(.N, size = round(.N * 0.8))], by = collusion]
train_data <- train_data %>% select(time_stamp, collusion, everything())
test_data <- fsetdiff(preprocessed, train_data)
...
save(train_data, test_data, file="2010_MSD_narrow.RData")
...
save(model, file="2010_MSD_narrow_model_0.RData")

# demeaned
model <- carteltraintest(collusion=train_data[,2], prices=train_data[,-c(1, 2)], nsim=5, partial=FALSE, numberbids=TRUE, trainshare=0.8, xstandardize=1)
model$results

save(model, file="2010_MSD_narrow_model_1.RData")

# standardized
model <- carteltraintest(collusion=train_data[,2], prices=train_data[,-c(1, 2)], nsim=5, partial=FALSE, numberbids=TRUE, trainshare=0.8, xstandardize=2)
model$results

save(model, file="2010_MSD_narrow_model_2.RData")
