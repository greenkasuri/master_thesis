...
# collusive partners market
df_relevant <- function(){df[UNIT_REFERENCE_NO %in% collusive_units &
                               PURPOSE_CD=="OFF" &
                               BILATERAL_IN=="FALSE" &
                               (STATUS_CD=="ACC" | STATUS_CD=="REJ") &
                               # collusion day (holiday) or Sunday
                               (BID_OFFER_DATE_DT %in% collusion_dates | wday(ymd(BID_OFFER_DATE_DT))==1)]}
...
save(equal_size_sample, file="2010_MGP_collusive_data.RData")
...
# demeaned
model_full_demeaned <- carteltraintest(collusion=equal_size_sample[,1], prices=equal_size_sample[,-1], nsim=100, partial=FALSE, numberbids=TRUE, xstandardize=2)
model_full_demeaned$results
...
save(model, file="2010_MGP_collusive_model.RData")
save(model_demeaned, file="2010_MGP_collusive_model_demeaned.RData")
save(model_standardized, file="2010_MGP_collusive_model_standardized.RData")
