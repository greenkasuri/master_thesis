...
save(train_data, test_data, file="2016_MSD_narrow.RData")
...
# train
model <- carteltraintest(collusion=train_data[,2], prices=train_data[,-c(1, 2)], partial=FALSE, numberbids=TRUE, trainshare=0.8)
model$results

save(model, file="2016_MSD_narrow_model_0.RData")

# demeaned
model <- carteltraintest(collusion=train_data[,2], prices=train_data[,-c(1, 2)], partial=FALSE, numberbids=TRUE, trainshare=0.8, xstandardize=1)
model$results

save(model, file="2016_MSD_narrow_model_1.RData")
...
save(model, file="2016_MSD_narrow_model_2.RData")
